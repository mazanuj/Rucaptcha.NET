...
namespace Akumu.Antigate.Tools
{
  ...
  {
    private static Regex ReCapChallenge = new Regex("challenge(?:\\s+)?:(?:\\s+)?'([a-zA-Z0-9_-]+)',");
    private static Regex ReCapChallengeLink = new Regex("(http://(?:www.)?google.com/recaptcha/api/challenge\\?k=(?:\\w+))");
    ...
    public static ReCaptchaObject GetObject(string ScriptPage)
    {
      ...
      if (match.Success)
        return new ReCaptchaObject(match.Groups[1].Value);
      return (ReCaptchaObject) null;
    }
  }
}
